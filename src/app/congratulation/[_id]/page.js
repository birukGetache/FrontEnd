...
// Dynamic import for map to improve performance
...
const BookingDetailsPage = () => {
  ...
  // Fetch weather data (mocked or use real API like OpenWeatherMap)
  useEffect(() => {
  ...
  // Destination content
  const destinationDescription = `Embark on an unforgettable journey to ${
    data.bookingDetails.destinationLocation
  }. Discover pristine beaches, vibrant coral reefs, and local culture that promises adventure and relaxation. Whether it's snorkeling in turquoise waters or savoring sunset views, this destination has it all.`;
  ...
  return (
    <div className={`min-h-screen    relative overflow-hidden  ${darkMode ? "bg-black": "bg-white" }`}>
      ...
      >
        <motion.button
          ...
          data-tooltip-content={t("BackToHome")}
        ...
        <div className="flex items-center space-x-4">
          <motion.button
            ...
            data-tooltip-content={t("DownloadCard")}
          ...
          {qrUrl && (
            <motion.button
              ...
              data-tooltip-content={t("ViewQRCode")}
          ...
          <motion.button
            ...
            data-tooltip-content={t("ShareBooking")}
    ...
    text-gray-500 font-semibold text-sm 
     shadow-black/10 bg-opacity-90 backdrop-blur-lg shadow-2xl rounded-3xl p-8 border-2 border-blue-200 hover:border-blue-400 transition-all duration-500"
        >
          <motion.h2
        ...
        {/* About Your Trip & Additional Info */}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700 font-medium">
              {[
                {
                  label: t("CheckInTime"),
                  value: data.bookingDetails.checkInTime || "10:00 AM",
                },
                {
                  label: t("CheckOutTime"),
                  value: data.bookingDetails.checkOutTime || "6:00 PM",
                },
                {
                  label: t("BookingDate"),
                  value: data.bookingDetails.bookingDate || new Date().toLocaleDateString(),
                },
                {
                  label: t("BoatType"),
                  value: data.bookingDetails.boatType || "Yacht",
                },
                {
                  label: t("TotalCost"),
                  value: data.bookingDetails.totalCost
                    ? `$${data.bookingDetails.totalCost}`
                    : "$500",
                },
              ].map((item, index) => (
                <motion.p
